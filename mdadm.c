...
int mdadm_read(uint32_t start_addr, uint32_t read_len, uint8_t *read_buf) {
    ...
    if (mount == 0 || read_len > 1024 || (read_buf == NULL && read_len != 0)) {
        return -1;
    }
    ...
    while (curr < last_addr) {
        ...
        if (cache_lookup(disk_id, block_id, buf_temp) != 1) {
            ...
            // Seek and read operations; abort the whole read on any failure
...
int mdadm_write(uint32_t start_addr, uint32_t write_len, const uint8_t *write_buf) {
    ...
    if (mount == 0 || write_len > 1024 || (write_buf == NULL && write_len != 0)) {
        return -1;
    }
    // check bounds
